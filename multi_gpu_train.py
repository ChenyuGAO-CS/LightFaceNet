...
def get_parser():
    ...
    parser.add_argument('--model_type', default=0,
                        help='MobileFaceNet or TinyMobileFaceNet or SqueezeFaceNet or ShuffleFaceNet')
...
def tower_loss(scope, images, labels, phase_train_placeholder, args):
    if args.model_type == 0:
        logits, net_points = MobileFaceNet.inference(images=images,
                                                     phase_train=phase_train_placeholder,
                                                     weight_decay=args.weight_decay)
    elif args.model_type == 1:
        logits, net_points = TinyMobileFaceNet.inference(images=images,
                                                         phase_train=phase_train_placeholder,
                                                         weight_decay=args.weight_decay)
    elif args.model_type == 2:
        logits, net_points = SqueezeFaceNet.inference(images=images,
                                                      phase_train=phase_train_placeholder,
                                                      weight_decay=args.weight_decay)
    else:
        logits, net_points = ShuffleFaceNet.inference(images=images,
                                                      phase_train=phase_train_placeholder,
                                                      weight_decay=args.weight_decay)
        ...
        for i in range(args.max_epoch):
            ...
            while True:
                try:
                    ...
                    if count > 0 and args.train_batch_size * count * len(gpu_group.split(',')) > 2830146:
